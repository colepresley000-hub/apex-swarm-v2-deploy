...
try:
    from slash_skills import SLASH_SKILLS, parse_slash_command, apply_skill, format_skill_help
    SLASH_SKILLS_AVAILABLE = True
    logger.info(f"✅ slash_skills loaded — {len(SLASH_SKILLS)} skills available")
except ImportError:
    logger.warning("⚠️ slash_skills not found")

# Agent Identity Containers
IDENTITY_AVAILABLE = False
identity_manager = None
try:
    from agent_identity import AgentIdentityManager, AgentIdentity, PERMISSION_SCOPES, PERMISSION_PRESETS
    IDENTITY_AVAILABLE = True
    logger.info("✅ agent_identity loaded — isolated agent containers ready")
except ImportError:
    logger.warning("⚠️ agent_identity not found")
...
try:
    ...
    logger.warning("⚠️ monte_carlo not found")
...
AGENT_CATEGORIES = {
    "Crypto & DeFi": {
        ...
        "agents": {
            ...
            "monte-carlo": {"name": "Monte Carlo Trader", "description": "Run 10,000 price simulations to find mispriced prediction markets", "system": "You are a quantitative trading agent specializing in Monte Carlo simulation. When given a prediction market question (e.g. \'Will BTC hit $100K by March 15?\'), you: 1) Pull historical volatility data and relevant macro factors, 2) Run probabilistic analysis across thousands of simulated scenarios, 3) Compare your probability estimate to current market odds, 4) Identify mispricings where math diverges from crowd sentiment, 5) Output a structured trade recommendation with entry, sizing, and rationale. You think in probability distributions, not point predictions. You factor in: historical price paths, macro regime, geopolitical risk premium, correlation with other assets, and time decay. Always output: estimated probability, market implied probability, edge (%), recommended position, and confidence level."},
...
CREATE TABLE IF NOT EXISTS usage_log (
                ...
                CREATE TABLE IF NOT EXISTS daemon_soul (
                    ...
                    improvement_log TEXT DEFAULT '[]',
                    last_improved_at TEXT,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL
                );

                CREATE TABLE IF NOT EXISTS daemon_brain (
                    daemon_id TEXT PRIMARY KEY,
                ...
                CREATE TABLE IF NOT EXISTS agent_identity (
                    ...
                    wallet_budget_usd REAL DEFAULT 0.0,
                    wallet_spent_usd REAL DEFAULT 0.0,
                    spending_limit_per_action REAL DEFAULT 10.0,
                    active INTEGER DEFAULT 1,
                    created_at TEXT NOT NULL,
    ...
    if IDENTITY_AVAILABLE:
        identity_manager = AgentIdentityManager(get_db, USER_KEY_COL)
        logger.info("✅ Identity manager initialized — agent containers active")

    # Add Monte Carlo trading daemon preset
    if MONTE_CARLO_AVAILABLE and MISSION_CONTROL:
        DAEMON_PRESETS["polymarket-hunter"] = {
            "name": "Polymarket Hunter",
            "description": "Scans prediction markets for Monte Carlo mispricings every hour",
            "agent_type": "monte-carlo",
            "interval_seconds": 3600,
            "task_description": "Scan for mispriced prediction markets. Analyze BTC, ETH, and gold price prediction markets. Run Monte Carlo simulations to find where market odds diverge from mathematical probability by >15%. Report top opportunities with edge %, recommended position size, and rationale.",
            "alert_conditions": ["high edge opportunity found"],
        }
...
async def _daemon_execute_fn(agent_id: str, agent_type: str, prompt: str, user_api_key: str = "daemon", daemon_id: str = None):
    """Execute function for daemon agents — injects SOUL context, then executes."""
    tagged_key = f"daemon:{user_api_key}" if not user_api_key.startswith("daemon:") else user_api_key

    # Inject SOUL context if this daemon has one
    if daemon_id:
        try:
            conn = get_db()
            try:
                row = conn.execute(
                    "SELECT mission, hard_rules, blockers, learnings FROM daemon_soul WHERE daemon_id = ?",
                    (daemon_id,)
                ).fetchone()
            finally:
                conn.close()
            ...
            if row:
                mission, hard_rules, blockers, learnings = row
                soul_context = f"""\n\n--- SOUL DOCUMENT ---
MISSION: {mission}
HARD RULES: {', '.join(json.loads(hard_rules or '[]'))}
ACTIVE BLOCKERS TO AVOID: {', '.join(json.loads(blockers or '[]')) or 'None'}
KEY LEARNINGS: {'; '.join(json.loads(learnings or '[]')[-5:]) or 'None'}
--- END SOUL ---\n"""
                prompt = soul_context + prompt
        except Exception as e:
            logger.warning(f"Soul injection failed for {daemon_id[:8] if daemon_id else '?'}: {e}")

    # Inject IDENTITY CONTAINER context
    if daemon_id and IDENTITY_AVAILABLE and identity_manager:
        try:
            identity = identity_manager.get_by_daemon(daemon_id)
            if identity:
                prompt = identity.to_prompt_context() + prompt
        except Exception as e:
            logger.warning(f"Identity injection failed for {daemon_id[:8] if daemon_id else '?'}: {e}")

    result = await execute_task(agent_id, agent_type, prompt, tagged_key)
    # Update BRAIN with cycle result
    if daemon_id and result:
        try:
            update_daemon_brain(daemon_id, str(result)[:500])
        except Exception:
            pass
    return result
...
def get_or_create_daemon_soul(daemon_id: str, user_api_key: str, agent_name: str, task_description: str) -> dict:
    """Get or initialize a daemon SOUL document."""
    conn = get_db()
    try:
        row = conn.execute("SELECT daemon_id, user_api_key, mission, hard_rules, blockers, learnings, improvement_log, last_improved_at FROM daemon_soul WHERE daemon_id = ?", (daemon_id,)).fetchone()
        if row:
            return {
                "daemon_id": row[0], "user_api_key": row[1], "mission": row[2],
                "hard_rules": json.loads(row[3] or "[]"),
                "blockers": json.loads(row[4] or "[]"),
                "learnings": json.loads(row[5] or "[]"),
                "improvement_log": json.loads(row[6] or "[]"),
                "last_improved_at": row[7],
            }
        now = datetime.now(timezone.utc).isoformat()
        soul = {
            "daemon_id": daemon_id, "user_api_key": user_api_key,
            "mission": f"I am {agent_name}. My mission: {task_description}",
            "hard_rules": ["Never fabricate data", "Always report failures honestly", "Stay focused on my mission"],
            "blockers": [], "learnings": [], "improvement_log": [], "last_improved_at": None,
        }
        conn.execute(
            "INSERT INTO daemon_soul (daemon_id, user_api_key, mission, hard_rules, blockers, learnings, improvement_log, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (daemon_id, user_api_key, soul["mission"], json.dumps(soul["hard_rules"]), "[]", "[]", "[]", now, now)
        )
        conn.commit()
        return soul
    finally:
...
def update_daemon_brain(daemon_id: str, result_summary: str):
    """Update daemon BRAIN after each execution cycle."""
    now = datetime.now(timezone.utc).isoformat()
    conn = get_db()
    try:
        row = conn.execute("SELECT cycle_count FROM daemon_brain WHERE daemon_id = ?", (daemon_id,)).fetchone()
        if row:
            conn.execute(
                "UPDATE daemon_brain SET last_result_summary=?, cycle_count=cycle_count+1, last_cycle_at=?, updated_at=? WHERE daemon_id=?",
                (result_summary[:500], now, now, daemon_id)
            )
        else:
            conn.execute(
                "INSERT INTO daemon_brain (daemon_id, active_tasks, priorities, last_result_summary, cycle_count, last_cycle_at, updated_at) VALUES (?,?,?,?,1,?,?)",
                (daemon_id, "[]", "[]", result_summary[:500], now, now)
            )
        conn.commit()
    finally:
        conn.close()
...
def save_daemon_soul(soul: dict):
    """Persist updated soul back to DB."""
    now = datetime.now(timezone.utc).isoformat()
    conn = get_db()
    try:
        conn.execute(
            "UPDATE daemon_soul SET mission=?, hard_rules=?, blockers=?, learnings=?, improvement_log=?, last_improved_at=?, updated_at=? WHERE daemon_id=?",
            (soul["mission"], json.dumps(soul.get("hard_rules", [])), json.dumps(soul.get("blockers", [])),
             json.dumps(soul.get("learnings", [])), json.dumps(soul.get("improvement_log", [])[-50:]),
             soul.get("last_improved_at"), now, soul["daemon_id"])
        )
        conn.commit()
    finally:
        conn.close()
...
async def run_self_improvement(daemon_id: str, daemon_name: str, task_description: str, user_api_key: str):
    """Run one self-improvement cycle for a daemon."""
    try:
        soul = get_or_create_daemon_soul(daemon_id, user_api_key, daemon_name, task_description)
        cutoff = (datetime.now(timezone.utc) - timedelta(hours=24)).isoformat()
        conn = get_db()
        try:
            rows = conn.execute(
                f"SELECT agent_type, task_description, status, result, created_at FROM agents WHERE {USER_KEY_COL} LIKE ? AND created_at > ? ORDER BY created_at DESC LIMIT 20",
                (f"%{user_api_key}%", cutoff)
            ).fetchall()
        finally:
            conn.close()

        if not rows:
            logger.info(f"Self-improve: no runs for daemon {daemon_id[:8]}")
            return

        transcript = "\n".join([
            f"[{r[4][:16]}] {r[0]} | status={r[2]} | result={str(r[3] or '')[:200]}"
            for r in rows
        ])

        improvement_prompt = f"""Analyze this daemon agent's recent execution history to improve its performance.

DAEMON: {daemon_name}
MISSION: {soul['mission']}
HARD RULES: {json.dumps(soul.get('hard_rules', []))}
KNOWN BLOCKERS: {json.dumps(soul.get('blockers', []))}
RECENT LEARNINGS: {json.dumps(soul.get('learnings', [])[-5:])}
...
{{
  "new_blockers": ["newly discovered blockers not already known"],
  "resolved_blockers": ["previously known blockers now resolved"],
  "new_learnings": ["actionable new learnings"],
  "mission_patch": "updated mission statement or null",
  "summary": "1-2 sentence summary of changes"
}}"""

        result = await router.call(
            system_prompt="You are a daemon self-improvement engine. Output only valid JSON, no markdown.",
        ...
        try:
            ...
            imp = json.loads(clean)
            blockers = soul.get("blockers", [])
            learnings = soul.get("learnings", [])
            for b in imp.get("new_blockers", []):
                if b and b not in blockers: blockers.append(b)
            for b in imp.get("resolved_blockers", []):
                if b in blockers: blockers.remove(b)
            for l in imp.get("new_learnings", []):
                if l and l not in learnings: learnings.append(l)
            if imp.get("mission_patch"):
                soul["mission"] = imp["mission_patch"]
            soul["blockers"] = blockers[-20:]
            soul["learnings"] = learnings[-30:]
            soul["last_improved_at"] = datetime.now(timezone.utc).isoformat()
            soul["improvement_log"] = soul.get("improvement_log", []) + [{
                "at": soul["last_improved_at"], "summary": imp.get("summary", ""),
                "new_blockers": len(imp.get("new_blockers", [])), "new_learnings": len(imp.get("new_learnings", [])),
            }]
            save_daemon_soul(soul)
            logger.info(f"Self-improved {daemon_name}: {imp.get('summary', '')}")
        except json.JSONDecodeError as e:
            logger.error(f"Self-improve parse error {daemon_id[:8]}: {e}")
    except Exception as e:
        logger.error(f"Self-improve error {daemon_id[:8]}: {e}")


async def self_improvement_loop():
    """Nightly loop: runs at 2am UTC, improves every active daemon."""
    ...
    while True:
        try:
            ...
            logger.info(f"Next self-improvement in {wait_seconds/3600:.1f}h")
            await asyncio.sleep(wait_seconds)
            conn = get_db()
            try:
                rows = conn.execute(
                    f"SELECT id, {USER_KEY_COL}, agent_name, task_description FROM daemon_configs WHERE enabled = 1"
                ).fetchall()
            finally:
                conn.close()
            logger.info(f"Running self-improvement for {len(rows)} daemons")
            for row in rows:
                daemon_id, user_api_key, agent_name, task_description = row
                await run_self_improvement(daemon_id, agent_name, task_description, user_api_key)
...
async def handle_telegram_message(message: dict):
    ...
    skill_key = None
    skill_data = None
    if SLASH_SKILLS_AVAILABLE and text.startswith("/"):
        skill_key, remaining = parse_slash_command(text)
        if skill_key:
            skill_data = apply_skill(skill_key, remaining)
            agent_type = skill_data["agent_type"] if skill_data["agent_type"] in AGENTS else "research"
            task = skill_data["system_prompt"] + "\n\n---\nTASK:\n" + remaining
        else:
    ...
    if agent_type == "skills":
        if SLASH_SKILLS_AVAILABLE:
            await send_telegram(chat_id, format_skill_help())
        else:
            await send_telegram(chat_id, "⚠️ Slash skills not loaded")
        return
...
async def start_daemon(req: DaemonRequest, api_key: str = Depends(get_api_key)):
    ...
    if req.preset_id and req.preset_id in DAEMON_PRESETS:
        ...
        # Auto-initialize soul document
        get_or_create_daemon_soul(daemon_id, api_key, preset["name"], task_desc)
        # Auto-create identity container
        if IDENTITY_AVAILABLE and identity_manager:
            identity_manager.create_container(
                owner_api_key=api_key, agent_name=preset["name"],
                agent_type=preset["agent_type"], mission=task_desc,
                daemon_id=daemon_id, permissions=PERMISSION_PRESETS.get("operator", ["read","write","search"]),
            )
        return {"daemon_id": daemon_id, "name": preset["name"], "status": "running", "persistent": True, "soul_initialized": True, "identity_created": True}
        ...
        # Auto-initialize soul document
        get_or_create_daemon_soul(daemon_id, api_key, req.agent_name, req.task_description)
        # Auto-create identity container
        if IDENTITY_AVAILABLE and identity_manager:
            identity_manager.create_container(
                owner_api_key=api_key, agent_name=req.agent_name,
                agent_type=req.agent_type, mission=req.task_description,
                daemon_id=daemon_id, permissions=PERMISSION_PRESETS.get("operator", ["read","write","search"]),
            )
        return {"daemon_id": daemon_id, "name": req.agent_name, "status": "running", "persistent": True, "soul_initialized": True, "identity_created": True}
    else:
...
async def get_daemon_soul(daemon_id: str, api_key: str = Depends(get_api_key)):
    """Get a daemon's SOUL document — mission, rules, blockers, learnings."""
    conn = get_db()
    try:
        row = conn.execute(
            "SELECT daemon_id, user_api_key, mission, hard_rules, blockers, learnings, improvement_log, last_improved_at FROM daemon_soul WHERE daemon_id = ?",
            (daemon_id,)
        ).fetchone()
    finally:
        conn.close()
    if not row:
        # Auto-create soul from daemon config
        conn = get_db()
        try:
            dc = conn.execute(f"SELECT id, {USER_KEY_COL}, agent_name, task_description FROM daemon_configs WHERE id = ?", (daemon_id,)).fetchone()
        finally:
            conn.close()
        if not dc:
            raise HTTPException(status_code=404, detail="Daemon not found")
        soul = get_or_create_daemon_soul(dc[0], dc[1], dc[2], dc[3])
        return soul
    return {
        "daemon_id": row[0], "mission": row[2],
        "hard_rules": json.loads(row[3] or "[]"),
        "blockers": json.loads(row[4] or "[]"),
        "learnings": json.loads(row[5] or "[]"),
        "improvement_log": json.loads(row[6] or "[]")[-10:],
        "last_improved_at": row[7],
    }


@app.post("/api/v1/daemons/{daemon_id}/soul")
async def update_daemon_soul(daemon_id: str, request: Request, api_key: str = Depends(get_api_key)):
    """Manually update a daemon's SOUL fields."""
    data = await request.json()
    conn = get_db()
    try:
        row = conn.execute("SELECT daemon_id FROM daemon_soul WHERE daemon_id = ?", (daemon_id,)).fetchone()
    finally:
        conn.close()
    if not row:
        raise HTTPException(status_code=404, detail="Soul not found — deploy the daemon first")
    now = datetime.now(timezone.utc).isoformat()
    conn = get_db()
    try:
        if "mission" in data:
            conn.execute("UPDATE daemon_soul SET mission=?, updated_at=? WHERE daemon_id=?", (data["mission"], now, daemon_id))
        if "hard_rules" in data:
            conn.execute("UPDATE daemon_soul SET hard_rules=?, updated_at=? WHERE daemon_id=?", (json.dumps(data["hard_rules"]), now, daemon_id))
        conn.commit()
    finally:
        conn.close()
    return {"status": "updated", "daemon_id": daemon_id}


@app.get("/api/v1/daemons/{daemon_id}/brain")
async def get_daemon_brain(daemon_id: str, api_key: str = Depends(get_api_key)):
    """Get a daemon BRAIN — active tasks, priorities, last result, cycle count."""
    conn = get_db()
    try:
        row = conn.execute(
            "SELECT daemon_id, active_tasks, priorities, last_result_summary, cycle_count, last_cycle_at FROM daemon_brain WHERE daemon_id = ?",
            (daemon_id,)
        ).fetchone()
    finally:
        conn.close()
    if not row:
        return {"daemon_id": daemon_id, "cycle_count": 0, "active_tasks": [], "priorities": [], "last_result_summary": "", "last_cycle_at": None}
    return {
        "daemon_id": row[0],
        "active_tasks": json.loads(row[1] or "[]"),
        "priorities": json.loads(row[2] or "[]"),
        "last_result_summary": row[3],
        "cycle_count": row[4],
        "last_cycle_at": row[5],
    }


@app.post("/api/v1/daemons/{daemon_id}/improve")
async def trigger_self_improvement(daemon_id: str, api_key: str = Depends(get_api_key)):
    """Manually trigger self-improvement for a daemon (don't wait for 2am)."""
    conn = get_db()
    try:
        dc = conn.execute(f"SELECT id, {USER_KEY_COL}, agent_name, task_description FROM daemon_configs WHERE id = ?", (daemon_id,)).fetchone()
    finally:
    ...
    if not dc:
        raise HTTPException(status_code=404, detail="Daemon not found")
    asyncio.create_task(run_self_improvement(dc[0], dc[2], dc[3], dc[1]))
    return {"status": "improvement_started", "daemon_id": daemon_id}
...
async def list_skills(api_key: str = Depends(get_api_key)):
    """List all available slash skills."""
    if not SLASH_SKILLS_AVAILABLE:
    ...
    return {
        "skills": {
            ...
            for k, v in SLASH_SKILLS.items()
        },
        "count": len(SLASH_SKILLS),
        "usage": "Prefix any task with a slash skill: /review <code> or /plan-ceo-review <idea>",
    }
...
async def create_identity(req: IdentityCreateRequest, api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    identity = identity_manager.create_container(
        owner_api_key=api_key, agent_name=req.agent_name, agent_type=req.agent_type,
        mission=req.mission, daemon_id=req.daemon_id, permissions=req.permissions,
        wallet_budget_usd=req.wallet_budget_usd, spending_limit_per_action=req.spending_limit_per_action,
    )
    d = identity.to_dict()
    d["credentials"] = {}
    return d
...
async def list_identities(api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        return {"containers": []}
    containers = identity_manager.list_containers(api_key)
    return {"containers": [{
        "container_id": c.container_id, "daemon_id": c.daemon_id,
        "agent_name": c.agent_name, "agent_type": c.agent_type,
        "email_alias": c.email_alias, "permissions": c.permissions,
        "wallet_remaining": c.wallet_remaining, "wallet_budget_usd": c.wallet_budget_usd,
        "wallet_spent_usd": c.wallet_spent_usd, "active": c.active,
        "created_at": c.created_at,
    } for c in containers]}

@app.get("/api/v1/identity/scopes")
async def list_permission_scopes(api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    return {"scopes": PERMISSION_SCOPES, "presets": PERMISSION_PRESETS}

@app.get("/api/v1/identity/{container_id}")
async def get_identity(container_id: str, api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    identity = identity_manager.get_by_id(container_id)
    if not identity or identity.owner_api_key != api_key:
        raise HTTPException(status_code=404, detail="Identity not found")
    d = identity.to_dict()
    d["credentials"] = {k: "configured" for k in json.loads(d["credentials"]).keys()}
    d["wallet_remaining"] = identity.wallet_remaining
    return d

@app.post("/api/v1/identity/{container_id}/credentials")
async def add_credential(container_id: str, req: CredentialAddRequest, api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    identity = identity_manager.get_by_id(container_id)
    if not identity or identity.owner_api_key != api_key:
        raise HTTPException(status_code=404, detail="Identity not found")
    success = identity_manager.add_credential(container_id, req.service, req.credential_data)
    return {"status": "added" if success else "failed", "service": req.service}

@app.post("/api/v1/identity/{container_id}/permissions")
async def update_permissions(container_id: str, request: Request, api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    identity = identity_manager.get_by_id(container_id)
    if not identity or identity.owner_api_key != api_key:
        raise HTTPException(status_code=404, detail="Identity not found")
    data = await request.json()
    identity_manager.update_permissions(container_id, data.get("permissions", []))
    return {"status": "updated", "permissions": data.get("permissions", [])}

@app.post("/api/v1/identity/{container_id}/fund")
async def fund_wallet(container_id: str, req: WalletFundRequest, api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    identity = identity_manager.get_by_id(container_id)
    if not identity or identity.owner_api_key != api_key:
        raise HTTPException(status_code=404, detail="Identity not found")
    if req.amount_usd <= 0 or req.amount_usd > 10000:
        raise HTTPException(status_code=400, detail="Amount must be between $0.01 and $10,000")
    new_balance = identity_manager.fund_wallet(container_id, req.amount_usd)
    return {"status": "funded", "new_balance_usd": new_balance}

@app.delete("/api/v1/identity/{container_id}")
async def deactivate_identity(container_id: str, api_key: str = Depends(get_api_key)):
    if not IDENTITY_AVAILABLE or not identity_manager:
        raise HTTPException(status_code=503, detail="Identity system not loaded")
    identity = identity_manager.get_by_id(container_id)
    if not identity or identity.owner_api_key != api_key:
        raise HTTPException(status_code=404, detail="Identity not found")
    identity_manager.deactivate(container_id)
    return {"status": "deactivated", "container_id": container_id}
...
class MonteCarloRequest(BaseModel):
    ...
    target_price: float
    deadline_days: float
    market_probability: float       # 0.0-1.0 (e.g. 0.18 for 18 cents)
    direction: str = "above"        # "above" or "below"
    simulations: int = 10000
    event_type: str = "close"       # "close" or "touch"
    macro_adjustment: float = 0.0


@app.post("/api/v1/monte-carlo/analyze")
async def monte_carlo_analyze(req: MonteCarloRequest, api_key: str = Depends(get_api_key)):
    """Run Monte Carlo simulation on a prediction market question."""
    if not MONTE_CARLO_AVAILABLE:
        raise HTTPException(status_code=503, detail="Monte Carlo engine not loaded")
    if not (0 < req.market_probability < 1):
        raise HTTPException(status_code=400, detail="market_probability must be between 0 and 1")
    if req.simulations > 50000:
        raise HTTPException(status_code=400, detail="Max 50,000 simulations per request")
    result = await run_monte_carlo_analysis(
        ...
        macro_adjustment=req.macro_adjustment,
    )
    return result


class MonteCarloScanRequest(BaseModel):
    questions: list[dict]   # [{question, market_probability}, ...]
    simulations: int = 5000


@app.post("/api/v1/monte-carlo/scan")
async def monte_carlo_scan(req: MonteCarloScanRequest, api_key: str = Depends(get_api_key)):
    """Batch scan multiple prediction markets for mispricings."""
    if not MONTE_CARLO_AVAILABLE:
        raise HTTPException(status_code=503, detail="Monte Carlo engine not loaded")
    if len(req.questions) > 20:
        raise HTTPException(status_code=400, detail="Max 20 questions per scan")
    opportunities = await scan_for_opportunities(req.questions, simulations=req.simulations)
    return {
        "total_scanned": len(req.questions),
        "opportunities_found": len(opportunities),
        "opportunities": opportunities,
    }


@app.get("/api/v1/monte-carlo/parse")
async def monte_carlo_parse(question: str, api_key: str = Depends(get_api_key)):
    """Parse a Polymarket question into simulation parameters."""
    if not MONTE_CARLO_AVAILABLE:
        raise HTTPException(status_code=503, detail="Monte Carlo engine not loaded")
    return parse_polymarket_question(question)
...
async def deploy_agent(req: DeployRequest, api_key: str = Depends(get_api_key)):
    ...
    agent_type = req.agent_type
    if SLASH_SKILLS_AVAILABLE:
        skill_key, task = parse_slash_command(task)
        if skill_key:
            skill_result = apply_skill(skill_key, task)
            task = skill_result["task"]
            skill_applied = skill_result["skill"]
            # Override agent_type if skill specifies one and user didn't specify a non-default
            if skill_result["agent_type"] and skill_result["agent_type"] in AGENTS:
                agent_type = skill_result["agent_type"]
            # Prepend skill system prompt to task
            task = skill_result["system_prompt"] + "\n\n---\nTASK:\n" + task
    ...
    if skill_applied:
        response["skill_applied"] = skill_applied["name"]
        response["skill_mode"] = skill_applied["mode"]
    return response
